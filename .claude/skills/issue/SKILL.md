...
- Always include a short explanation of **what value solving this brings** (business, UX, technical, or maintainability impact).
- Distinguish between `feature`, `enhancement`, `refactor`, `bug`, and `chore`.
- Recommend appropriate existing `area:` labels (e.g., `area:ui`, `area:cms`, `area:i18n`, `area:build`, `area:architecture`).
- Avoid introducing new labels unless truly necessary; prefer reusing existing project labels and only suggest additions when justified.
